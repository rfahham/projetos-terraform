...
resource "aws_launch_template" "workon" {
  name_prefix   = local.name
  image_id      = "ami-0ae8f15ae66fe8cda" #specific for each region
  ...
  network_interfaces {
    ...
    security_groups             = [aws_security_group.workon_sg_for_ec2.id]
  }
  tag_specifications {
...
resource "aws_autoscaling_group" "sh_asg" {
  ...
  launch_template {
    id      = aws_launch_template.workon.id
    version = "$Latest"
  }
}
